    ...
    isAdmin()
      ...
      : h('section', { class: 'sec' },
        ...
        ...(pending.filter((a) => a.createdBy === me.name).length
          ? pending.filter((a) => a.createdBy === me.name).map((a) => activityCard(a, { onChange: refresh }))
          : [h('div', { class: 'empty' }, '대기 중인 제출이 없습니다.')])),

    h('section', { class: 'sec' },
      h('div', { class: 'sec-head' }, h('h3', {}, isAdmin() ? '최근 처리 이력' : '내 최근 처리 결과')),
      isAdmin() ? auditTable() : (mine.length
        ? h('div', {}, ...mine.map((a) => activityCard(a, { compact: true, onChange: refresh })))
    ...
    h('div', { class: 'pending-actions' },
      ...
      h('button', {
        class: 'btn btn-sm btn-danger', onClick: async () => {
          ...
          await audit('반려', a.id, a, { ...a, status: 'rejected', rejectReason: reason });
          toast('반려 처리했습니다.', 'ok');
          refresh();
        },
      }, '반려'),
...
async function approve(a, me) {
  const after = { ...a, status: 'approved', rejectReason: '', reviewedBy: me.name, reviewedAt: new Date().toISOString() };
